...
<!-- 		<view class="bottom-back">
			<view class="bottom-area">
				<view v-if="showSelectTag" class="conversation-bubble" @tap.stop="handleEditToggle">
					<view v-for="(item, index) in array" :key="index" class="picker" 
					:data-name="item.name" @tap="handleOnTap">{{ item.name }}</view>
				</view>
...
import TUIConversationItem from '@/pages_message/components/tui-conversation/conversation-item/index';
...
export default {
	...
	methods: {
		...
		// 监听函数，监听到后就直接把conversationList等于event.data
